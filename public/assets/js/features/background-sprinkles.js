...
function createIconPlacement(icon, zone, viewportWidth) {
  ...
  return {
    ...
    driftY: randomBetween(-5, 5),
    driftRotation: randomBetween(-2.8, 2.8),
    floatDuration: randomBetween(30, 52),
...
function createStarPlacement(zone, viewportWidth) {
  ...
  return {
    ...
    twinkleDuration: randomBetween(7.5, 13.5),
...
function createCloudPlacement(zone, viewportWidth) {
  ...
  return {
    ...
    driftX: randomBetween(-11, 11),
    driftY: randomBetween(-3, 4),
    floatDuration: randomBetween(42, 68),
...
export function initBackgroundSprinkles(options = {}) {
  ...
  const enabledTypes = new Set(
    Array.isArray(options.types) && options.types.length > 0 ? options.types : ["cloud", "star", "icon"],
  );
  ...
  function renderSprinkles() {
    ...
    const cloudPlacements = enabledTypes.has("cloud") ? buildCloudPlacements(zones.cloud, viewportWidth) : [];
    const iconPlacements = enabledTypes.has("icon")
      ? buildIconPlacements(zones.icon, zones.sideWidths, viewportWidth)
      : [];
    const starPlacements = enabledTypes.has("star")
      ? buildStarPlacements(
          zones.star,
          zones.sideWidths,
          viewportWidth,
