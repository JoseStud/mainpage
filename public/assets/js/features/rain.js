...
export function initRain() {
  ...
  function drawRain(now = performance.now()) {
    ...
    const deltaMs = lastFrameAt > 0 ? Math.max(0, now - lastFrameAt) : 1000 / 60;
    const deltaRatio = Math.min(2, deltaMs / (1000 / 60));
    lastFrameAt = now;
    ...
    drops.forEach((drop) => {
      ...
      drop.x -= drop.drift * 0.22 * deltaRatio;
      drop.y += drop.speed * deltaRatio;
