...
- Shared shell structure mounted around each page at runtime:
  - `canvas#rain-canvas`
  - `.page`
  - `.site-header`
  - `main.layout`
...
- `controls`: local rain toggle and theme note rendered into the shared shell

Shared content for those widgets now lives in `assets/js/config/shared-shell.js` and `assets/js/config/page-shells.js`, with `assets/js/config/shell.js` acting as the lookup entrypoint.
